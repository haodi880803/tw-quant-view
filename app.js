...
(() => {
  ...
  const S = {
    ...
    loadingSymbol: false,
    healthLoadedAt: 0,
    shadowChart: null,     // 影子對照 equity chart
    shadowLoadedAt: 0,
    shadowComputing: false,
  ...
  async function viewerApi(path, opts) {
    ...
    if (p.startsWith('/api/shadow')) return viewerFetchJson('shadow.json');
  ...
  function tickCountdown() {
    ...
    // scan-kind label (2026-07-29): "14:15:21" alone reads as "the closing
    // session was never scanned" — name what kind of scan it was
    $('#last-scan').textContent = fmtClock(e.last_scan) +
      (e.last_scan && e.last_scan_kind ? '(' + e.last_scan_kind + ')' : '');
    const el = $('#next-scan');
  ...
  async function loadShadow() {
    ...
    S.shadowLoadedAt = Date.now();
    const empty = $('#shadow-empty');
    const content = $('#shadow-content');
    if (!r.ok || !r.data) {
      empty.classList.remove('hidden');
      empty.textContent = '無法載入影子對照資料';
  ...
  function clsAB(v) {  // arm palette for B−A columns (never 漲跌 red/green)
    return v > 0 ? 'b-ahead' : v < 0 ? 'a-ahead' : 'flat';
  }

  function shadowLegChips(legs) {
    const chip = (label, leg, detail) => {
      if (!leg) return '<span class="leg-chip pending">' + label + ':—(等待資料)</span>';
      const mark = leg.pass ? '✓' : '✗';
      const cls = leg.pass ? 'leg-chip ok' : 'leg-chip pending';
      return '<span class="' + cls + '" title="四腿於結案時整窗判定;觀察期未過屬預期">'
        + label + ' ' + mark + ' <span class="dim">' + escapeHtml(detail) + '</span></span>';
    };
    const op = legs.operational, sf = legs.safety, rt = legs.return, rl = legs.realization;
    return [
      chip('操作腿', op, op ? 'prefix_breaks=' + op.prefix_breaks : ''),
      chip('安全腿', sf, sf ? 'MDD B ' + (sf.mdd_b * 100).toFixed(1) + '% / A ' + (sf.mdd_a * 100).toFixed(1) + '%' : ''),
      chip('報酬腿', rt, rt ? 'B−A ' + fmtPp(rt.diff * 100) : ''),
      chip('實現腿', rl, rl ? 'B 滑價 ' + rl.slip_bp_b + 'bp(門檻 max(2×A, 5bp))' : ''),
    ].join('');
  }

  function fmtGene(key, v) {
    if (v == null || !isFinite(v)) return '—';
    if (/_pct$/.test(key) || key === 'risk_frac') return (v * 100).toFixed(2) + '%';
  ...
  function renderShadow(d) {
    const empty = $('#shadow-empty');
    const content = $('#shadow-content');
    const stale = $('#shadow-stale');
    stale.classList.toggle('hidden', !d.computing);
    if (d.computing && d.refresh_failed) {
      stale.textContent = '更新失敗';
      stale.title = '面板重烤失敗,已寫入運作日誌;每 10 分鐘自動重試';
    } else {
      stale.textContent = '更新中…';
      stale.title = '面板資料與影子帳最新狀態不一致,背景重烤中';
    }
    S.shadowComputing = !!d.computing;

    if (!d.active) {
      empty.classList.remove('hidden');
      empty.textContent = d.reason || 'shadow 尚未起錶';
      content.classList.add('hidden');
      return;
    }

    // warnings (chain/freeze problems are display-level here; the frozen
    // adjudicator is the authority)
    const warn = $('#shadow-warn');
    const warns = [];
    if (d.chain_ok === false) warns.push('帳本雜湊鏈驗證失敗');
    if (d.freeze_ok === false) warns.push('凍結封條驗證失敗');
    (d.warnings || []).forEach((w) => warns.push(w));
    warn.classList.toggle('hidden', !warns.length);
    warn.textContent = warns.length ? '⚠ ' + warns.join('；') : '';

    // sealed verdict banner (only after the frozen close-out adjudicates)
    const verdictEl = $('#shadow-verdict');
    if (d.verdict) {
      verdictEl.classList.remove('hidden');
      verdictEl.textContent = '裁決(' + (d.verdict.adjudicated || '') + '):'
        + (d.verdict.pass ? '通過' : '未通過')
        + (d.verdict.capped ? '(120 日硬上限)' : '')
        + ' → ' + (d.verdict.next || '');
    } else {
    ...
    const co = d.closeout || {};
    $('#shadow-progress').textContent = '第 ' + (prog.n_days || 0) + '/'
      + (co.min_days || 60) + ' 交易日 ・ B 平倉 ' + (prog.closed_b || 0)
      + '/' + (co.target_closed || 10);

    if (!rows.length) {
      empty.classList.remove('hidden');
      empty.textContent = '影子帳等待首行(day i 決策、i+1 成交;窗內需 ≥2 交易日)';
      content.classList.add('hidden');
      $('#shadow-brief').textContent = '起始日 ' + (d.start_date || '—');
      return;
    }
    ...
    const armA = (d.arms && d.arms.a) || 'A';
    const armB = (d.arms && d.arms.b) || 'B';
    $('#shadow-brief').textContent =
      'A 現任(' + armA + ')' + fmtMoney(last.eq_a) + '(' + fmtPct(last.ret_a_pct) + ')'
      + ' ・ B 候選(' + armB + ')' + fmtMoney(last.eq_b) + '(' + fmtPct(last.ret_b_pct) + ')'
      + ' ・ 差 ' + fmtPp(last.diff_pp);

    $('#shadow-legs').innerHTML = shadowLegChips(d.legs || {});

    // equity chart — neutral hues on purpose: A/B is incumbent-vs-candidate,
    // not up-vs-down, so the TW red/green convention must not apply here
    if (typeof echarts !== 'undefined') {
      if (!S.shadowChart) S.shadowChart = echarts.init($('#shadow-chart'));
      S.shadowChart.setOption({
        animation: false,
        backgroundColor: 'transparent',
        legend: { top: 0, left: 0, textStyle: { color: AXIS_TEXT, fontSize: 10 } },
        tooltip: {
          trigger: 'axis',
          backgroundColor: 'rgba(23,29,41,0.95)', borderColor: '#232b3a',
          textStyle: { color: '#dfe4ee', fontSize: 11 },
          valueFormatter: (v) => (v == null ? '—' : Number(v).toLocaleString('en-US')),
          confine: true,
        },
        grid: { left: 70, right: 14, top: 24, bottom: 20 },
        xAxis: {
        ...
        yAxis: {
          ...
          axisLabel: { color: AXIS_TEXT, fontSize: 9, formatter: (v) => (v / 10000).toFixed(1) + '萬' },
          splitLine: { lineStyle: { color: GRID_LINE } },
        },
        series: [
          { name: 'A 現任', type: 'line', data: rows.map((r) => r.eq_a),
            symbol: 'circle', symbolSize: 4,
            lineStyle: { width: 2, color: ACCENT }, itemStyle: { color: ACCENT } },
          { name: 'B 候選', type: 'line', data: rows.map((r) => r.eq_b),
            symbol: 'circle', symbolSize: 4,
            lineStyle: { width: 2, color: '#c98500' }, itemStyle: { color: '#c98500' } },
        ],
      }, true);
      S.shadowChart.resize();
    }

    // daily ledger table, newest first (the chart carries chronology)
    $('#shadow-rows-body').innerHTML = rows.slice().reverse().map((r) => '<tr>'
      + '<td class="num dim">' + escapeHtml(r.date) + '</td>'
      + '<td class="r num">' + fmtMoney(r.eq_a) + '</td>'
      + '<td class="r num">' + fmtMoney(r.eq_b) + '</td>'
      + '<td class="r num ' + clsOf(r.ret_a_pct) + '">' + fmtPct(r.ret_a_pct) + '</td>'
      + '<td class="r num ' + clsOf(r.ret_b_pct) + '">' + fmtPct(r.ret_b_pct) + '</td>'
      // diff columns use the arm palette, NOT red/green: +pp only means
      // candidate B is ahead of incumbent A, not that money was made
      + '<td class="r num ' + clsAB(r.diff_pp) + '">' + fmtPp(r.diff_pp) + '</td>'
    ...
    const merged = [];
    (d.trades && d.trades.a || []).forEach((t) => merged.push({ arm: 'A', ...t }));
    (d.trades && d.trades.b || []).forEach((t) => merged.push({ arm: 'B', ...t }));
    merged.sort((x, y) => (x.date < y.date ? 1 : x.date > y.date ? -1 : x.arm.localeCompare(y.arm)));
    if (!merged.length) {
      tradesBody.innerHTML = '<tr><td colspan="7" class="empty-cell">窗內尚無交易</td></tr>';
    } else {
      tradesBody.innerHTML = merged.map((t) => {
        const isBuy = String(t.type).toUpperCase().startsWith('BUY');
        const note = isBuy
          ? (t.score != null ? '評分 ' + t.score.toFixed(1) : '')
          : ((t.reason ? escapeHtml(t.reason) : '') + (t.ret != null ? ' ・ 報酬 ' + fmtRet(t.ret) : ''));
        return '<tr>'
          + '<td class="c"><b>' + t.arm + '</b></td>'
          + '<td class="num dim">' + escapeHtml(t.date) + '</td>'
          + '<td>' + escapeHtml(String(t.symbol || '').replace(/\.(TW|TWO)$/i, '')) + '</td>'
          + '<td class="c"><span class="' + (isBuy ? 'up' : 'down') + '">' + (isBuy ? '買' : '賣') + '</span></td>'
          + '<td class="r num">' + fmtPrice(t.price) + '</td>'
          + '<td class="r num">' + (t.qty != null ? t.qty.toLocaleString('en-US') : '—') + '</td>'
          + '<td>' + note + '</td>'
          + '</tr>';
      }).join('');
    }

    // genome comparison (highlight = the arms differ on that gene)
    $('#shadow-genes-body').innerHTML = (d.genes || []).map((g) => '<tr'
      + (g.differs ? ' class="gene-diff-row"' : '') + '>'
      + '<td class="num">' + escapeHtml(g.gene) + '</td>'
      + '<td class="r num">' + fmtGene(g.gene, g.a) + '</td>'
      + '<td class="r num">' + fmtGene(g.gene, g.b) + '</td>'
      + '</tr>').join('');
  }
  ...
  function bindUI() {
    ...
    window.addEventListener('resize', () => {
      ...
      if (S.shadowChart) S.shadowChart.resize();
    });
  }
  ...
  async function init() {
    ...
    setInterval(() => {                    // keep shadow tab fresh while open;
      // a stale panel (computing) re-polls quickly to pick up the fresh bake
      if (S.bottomTab !== 'shadow') return;
      const maxAge = S.shadowComputing && !VIEWER ? 20000 : (VIEWER ? 300000 : 60000);
      if (Date.now() - S.shadowLoadedAt > maxAge) loadShadow();
    }, 10000);
  }
